...
static void my_touchpad_read(lv_indev_drv_t * indev_drv, lv_indev_data_t * data)
{
    //printf("my_touchpad_read \r\n");
    uint16_t touchX, touchY;
    bool touched = _lcd->getTouch( &touchX, &touchY);
    //bool touched = _lcd->isTouched()
    //if( !digitalRead(5) )
    if( !touched )
    {
...
static void encoder_read(lv_indev_drv_t * indev_drv, lv_indev_data_t * data)
{
    ...
    //printf("data->enc_diff : %d \r\n",data->enc_diff);
    //printf("isPush : %d \r\n",isPush);
    if(isPush != lastState)
    {
        //HAL::Buzz_Tone(isPush ? 500 : 700, 20);
        printf("isPush != lastState \r\n");
        lastState = isPush;
    }
}
#endif
