...
void SystemInfosModel::GetSA818Info(
    int* channel,
    float* freq_rx,
    float* freq_tx,
    int* volume,
    int* squelch,
    char* status, uint32_t len)
{
    HAL::SA818_Info_t sa818 = {0};
    account->Pull("SA818", &sa818, sizeof(sa818));
    
    *channel = sa818.channel;
    *freq_rx = sa818.freq_rx;
    *freq_tx = sa818.freq_tx;
    *volume = sa818.volume;
    *squelch = sa818.squelch;
    
    // 檢查 SA818 是否初始化成功
    // 如果頻率為 0 或 channel 為 0，表示初始化失敗
    if (sa818.freq_rx > 0 && sa818.freq_tx > 0 && sa818.channel > 0) {
        strncpy(status, "READY", len);
    } else {
        strncpy(status, "ERROR", len);
    }
    status[len - 1] = '\0';
}
