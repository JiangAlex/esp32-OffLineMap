...
{
    printf("WaveTable: Starting page creation\n");
    ...
    Btn_Create(ui.cont, &ui.btnH, "SA818", LV_SYMBOL_CALL);
    */
    
    // Focus the first button for encoder input and clear any old focus
    lv_group_t * group = lv_group_get_default();
    if(group && ui.btnA.btn) {
        // Remove all objects from group first to avoid conflicts
        lv_group_remove_all_objs(group);
        printf("WaveTable: Cleared all objects from group\n");
        
        // Re-add only the buttons we created
        lv_group_add_obj(group, ui.btnA.btn);
        lv_group_add_obj(group, ui.btnB.btn);
        lv_group_add_obj(group, ui.btnC.btn);
        /*
        lv_group_add_obj(group, ui.btnG.btn);
        lv_group_add_obj(group, ui.btnD.btn);
        lv_group_add_obj(group, ui.btnE.btn);
        lv_group_add_obj(group, ui.btnF.btn);
        lv_group_add_obj(group, ui.btnH.btn);
        */
        
        // Focus the first button
        lv_group_focus_obj(ui.btnA.btn);
        printf("WaveTable: Re-organized group with 8 buttons, first button focused=%p\n", ui.btnA.btn);
        
        // Check group status
        uint32_t obj_count = lv_group_get_obj_count(group);
        lv_obj_t * focused = lv_group_get_focused(group);
        printf("WaveTable: Group has %d objects, focused=%p\n", obj_count, focused);
    }
    
    printf("WaveTable: Page creation completed\n");
}

void WaveTableView::ResetEncoderGroup()
{
    printf("WaveTable: ResetEncoderGroup called\n");
    lv_group_t * group = lv_group_get_default();
    if(group && ui.btnA.btn) {
        // Remove all objects from group first
        lv_group_remove_all_objs(group);
        printf("WaveTable: ResetEncoderGroup - Cleared all objects from group\n");
        ...
        */
        ...
        // Focus the first button
        lv_group_focus_obj(ui.btnA.btn);
        printf("WaveTable: ResetEncoderGroup - Re-organized group with 8 buttons, focused=%p\n", ui.btnA.btn);
        
        // Check group status
        uint32_t obj_count = lv_group_get_obj_count(group);
        lv_obj_t * focused = lv_group_get_focused(group);
        printf("WaveTable: ResetEncoderGroup - Group has %d objects, focused=%p\n", obj_count, focused);
    }
}

void WaveTableView::Delete()
{
    // Clean up group when leaving this page
    lv_group_t * group = lv_group_get_default();
    if(group) {
        lv_group_remove_all_objs(group);
        printf("WaveTable: Delete - Cleared group objects\n");
    }
...
void WaveTableView::BtnCont_Create(lv_obj_t *par) // 按钮容器画布的创建
{
    ...
    // Don't add container to group - let buttons handle encoder input
    // Container will scroll automatically when buttons get focus
    lv_obj_add_flag(cont, LV_OBJ_FLAG_SCROLLABLE);         // Make sure it's scrollable
    lv_obj_add_flag(cont, LV_OBJ_FLAG_SCROLL_ON_FOCUS);    // Enable scroll when focused
    
    printf("WaveTable: Container configured for button-based encoder navigation\n");
...
void WaveTableView::Btn_Create(lv_obj_t *par, btnCont *cont, const char *name, const void *img_src)
{
    ...
    // Don't add to group here - will be added later in Create() function
    printf("WaveTable: Button '%s' created\n", name);
    
    // Add click event for encoder button press
    lv_obj_add_event_cb(btn, [](lv_event_t * e) {
        lv_event_code_t code = lv_event_get_code(e);
        lv_obj_t * btn = lv_event_get_target(e);
        if(code == LV_EVENT_CLICKED) {
            printf("WaveTable: Button clicked via encoder!\n");
            // Here you would add the actual button action
            // For now, just print which button was pressed
            // You can add specific actions based on the button
        }
        else if(code == LV_EVENT_FOCUSED) {
            printf("WaveTable: Button focused\n");
        }
    }, LV_EVENT_ALL, NULL);
